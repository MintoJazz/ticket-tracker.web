...
import type { TicketResponse, GetTicketsResponse } from "@/types";
import { failed } from "@/types/result";

export const ticketService = {
    findMany: async() => await request(api.get<GetTicketsResponse>('/tickets')),
    findOneById: async(id: string) => await request(api.get<TicketResponse>(`/tickets/${id}`)),
    updateById: async(id: string, formData: unknown) => {
        ...
        return await request(api.patch<TicketResponse>(`/tickets/${id}`, data))
    },
    create: async(formData: unknown) => {
        const { success, data, error } = CreateTicketSchema.safeParse(formData)
        if (!success) return failed(error)
        return await request(api.post<TicketResponse>('/tickets', data))
    }
}